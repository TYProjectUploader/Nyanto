...
public class ExplosiveUINavigationInGame : MonoBehaviour
//UI navigation with anything other than mouse
{
    ...
    void Update()
    {
        ...
        {
            ExplosivePauseRelatedButtons.instance.PauseScreen();
        }
        //if not paused or game over the following controls are ignored
        if (!ExplosivePauseRelatedButtons.instance.Paused && !ExplosiveGameManager.instance.gameOver)
        {
        ...
        else if (Navigation.action.WasPressedThisFrame() && !defaultSelected)
        {
            ...
            if (ExplosivePauseRelatedButtons.instance.Paused)
            {
