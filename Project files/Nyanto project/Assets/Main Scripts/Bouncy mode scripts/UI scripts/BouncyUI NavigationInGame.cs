...
public class BouncyUINavigationInGame : MonoBehaviour
//UI navigation with anything other than mouse
{
    ...
    void Update()
    {
        ...
        {
            BouncyPauseRelatedButtons.instance.PauseScreen();
        }
        //if not paused or game over the following controls are ignored
        if (!BouncyPauseRelatedButtons.instance.Paused && !BouncyGameManager.instance.gameOver)
        {
        ...
        else if (Navigation.action.WasPressedThisFrame() && !defaultSelected)
        {
            ...
            if (BouncyPauseRelatedButtons.instance.Paused)
            {
            ...
            if (BouncyGameManager.instance.gameOver)
            {
