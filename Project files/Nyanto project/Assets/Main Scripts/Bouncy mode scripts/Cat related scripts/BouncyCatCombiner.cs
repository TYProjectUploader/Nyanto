...
public class BouncyCatCombiner : MonoBehaviour //handles sending calls to combination manager
{
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("Cat") && !BouncyCombinationManager.instance.handlingCombine)
        {
            int otherCatIndex = collision.gameObject.GetComponent<BouncyCatCombiner>().thisCatIndex;
            ...
            if (otherCatIndex == thisCatIndex) //only call handle collision if both are the same to reduce load
            {
                ...
                BouncyCombinationManager.instance.HandleCollision(gameObject);
            }
        }
    }
