...
public class ExplosivePlayerMovement : MonoBehaviour
{
    ...
    */
    public static ExplosivePlayerMovement instance;
    ...
    void CheckDropAbility() //check if player dropped cat and can drop cat (determined by collderinformer when cat has made first contact)
    {
        if (canDrop && coolDownOver)
        {
            ...
            ExplosiveCatGenerator.instance.currentCat.SetActive(true); //display the next preloaded cat
            if (Drop.action.WasPressedThisFrame())
            {
                ...
                canDrop = false;
                coolDownOver = false;
                ExplosiveCatGenerator.instance.SimulateCurrentCat(); //simulate the current cat being held to make it drop
